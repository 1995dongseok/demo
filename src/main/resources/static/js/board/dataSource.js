...
class cpDataSource {
    ...
    set method(value) {
        if (Object.values(METHOD).includes(value)) this._method = value; else this._method = METHOD.GET;
    }
                ...
                read: {
                    ...
                    if (type === 'read') {
                        if (Object.keys(data.param).length > 0 && (data.param.constructor === Object || data.param.constructor === Array)) {
                            if (data.method !== METHOD.GET) return JSON.stringify(data.param); else return Object.entries(data.param).map(e => e.join('=')).join('&');
                        }
                        return '';
                    }
                }
